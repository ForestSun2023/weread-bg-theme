...
| 3 | **像素验证** | 15 种组合实拍，页边三段色值对官方实测，另验云的走向 / 月亮位置 / 色调方向 |
| 4 | **文档一致性** | 版本号四处对齐（脚本 / README 徽标 / CHANGELOG / 使用说明）；文档声称的取值与代码实际 id 一致 |
| 5 | **仓库卫生** | 脚本 BOM 齐全、敏感文件（HAR/快照/APK）确实被忽略、无超大跟踪文件、体积在脚本站 2 MB 限制内 |
| 6 | **死代码检查** | 函数名全文只出现 1 次 = 只有定义没有调用 |
| 7 | **体积** | 与上次记录对比并报增量（本项目**以字节数为准**，行数指标不可信） |

> **每一关都是被真实事故逼出来的。** 第 3 关是因为 v1.8.0「计算样式全绿、观感却很差」；
> 第 4 关是因为 GreasyFork 文档里把 `wrbg.backgrounds` 的 id 写成了 `sky`（实际 `moon`），
> 以及 `使用说明.md` 的版本号停在 v3.1.0 而脚本已经到 v3.2.0 —— 这种"代码对、文档错"的问题
> 不会让任何测试变红。第 5 关是因为 `.ps1` 两次丢掉 UTF-8 BOM 导致工具整个跑不起来。
>
> 一条贯穿所有关卡的设计原则：**「查不到断言目标」也算失败**。否则文档一改结构、
> 页面一改布局，关卡就会"静默全绿"，而静默的绿比红危险得多。

### 准备测试夹具

第 2、3 关需要**阅读页快照**（`.html`），它们**不在仓库里** —— 快照包含整章小说正文，属于版权内容。
